...
description: Configure the compiled Hiroshi daemon to run unattended in the background as a persistent system service.
---
...
Run the following command in an service terminal configuration window (Administrator PowerShell):
```powershell
...
- **Arguments**: Set this to `daemon` (required to run persistent background services and detach interactive stdin).
...
ExecStart=/home/yourusername/Hiroshi/target/release/Hiroshi daemon
Restart=always
RestartSec=5
LimitNOFILE=65536
